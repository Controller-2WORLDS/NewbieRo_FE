...
**서비스명**: 뉴비로 — 초보운전자를 뜻하는 '뉴비(newbie)'와 길을 뜻하는 '로(路)'를 합친 이름 (구 명칭: 안심로/SafeRo)
...
| 위험구간 | GET    | /api/risk-segments                        | 사고다발구간 목록/히트맵 조회   | bbox, time_band, accident_type                                     | segments[](segment_id, segment_type, road_name, admin_dong_cd, route_mile_range, start/end 좌표, accident_count, severity_score) |
