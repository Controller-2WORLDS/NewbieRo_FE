...
export function Dropdown<T extends string>({ label, options, value, onChange, align = "left" }: DropdownProps<T>) {
            ...
            <AnimatePresence>
                {open ? (
                    <motion.ul
                        ...
                        className={[
                            "absolute top-10.5 z-30 min-w-33 overflow-hidden rounded-2xl border border-line bg-grad-surface p-1 shadow-float-inset backdrop-blur-xl",
                    ...
                    >
                        {options.map((option) => (
                            <li key={option}>
                                ...
                                >
                                    {option}
                                    {option === value ? <CheckIcon className="h-4 w-4" strokeWidth={2.2} /> : null}
                                </button>
                            </li>
                        ))}
                    </motion.ul>
