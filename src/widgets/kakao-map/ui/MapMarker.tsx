...
export function MapMarker({ kind, position, active = false, delay = 0, label, onClick, children }: MapMarkerProps) {
    const Icon = kind === "risk" ? AlertTriangleIcon : kind === "place" ? MapPinIcon : MoonIcon
    const tone = kindTone[kind]

    return (
        <CustomOverlayMap position={position} yAnchor={1} zIndex={active ? 20 : 10}>
            <motion.div
